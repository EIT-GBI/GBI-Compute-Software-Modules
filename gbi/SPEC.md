...
or cloud credentials. Slurm executes directly as that user.
...
- `cli.py`: request, streaming selection, bounded worker supervision, history.
...
metadata calls, write close and readback. rclone supplies a sequential input
stream. The worker hashes while writing to a newly created final-name file,
...
The orchestrator holds at most the configured file concurrency in memory and
uses a depth-first scandir iterator. Each unfinished file has a small journal.
...
Interactive commands follow the submitted job. Ctrl-C detaches the viewer;
